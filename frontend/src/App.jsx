...
const GlobalStyles = () => (
  <style>{`
    ...
    * { box-sizing: border-box; }
    ...
    .main-title { margin: 0; font-size: 16px; font-weight: 700; letter-spacing: 0.3px; }
    .sub-title { font-size: 11px; opacity: 0.65; font-weight: 500; }
    ...
    .empty-state {
      ...
      height: 100%; text-align: center; opacity: 0.75;
    }
    .empty-title {
      font-size: 42px; font-weight: 800; margin: 0 0 12px;
    ...
    .msg-actions {
      display: flex; gap: 8px; margin-top: 6px; opacity: 0;
    ...
    .attachment-pill button { background: none; border: none; color: rgba(255,255,255,0.6); cursor: pointer; font-size: 14px; }
    ...
    .chat-action:hover { color: var(--primary); }
    .sidebar-footer { padding: 10px; font-size: 11px; opacity: 0.5; text-align: center; }
    
    .panel-overlay {
      position: absolute; inset: 0; background: rgba(0,0,0,0.4); z-index: 60; cursor: pointer;
    }
    .side-panel {
    ...
    .setting-label { font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; opacity: 0.7; margin-bottom: 10px; }
    .custom-input {
...
.model-select optgroup {
  background: #0f0518; color: rgba(255,255,255,0.7);
    ...
    .memory-card-title { font-size: 13px; font-weight: 600; margin-bottom: 10px; }
    .admin-title { font-size: 13px; font-weight: 600; margin-bottom: 14px; opacity: 0.8; }
    .admin-user-card {
    ...
    .camera-controls { display: flex; gap: 16px; margin-top: 20px; }
    .camera-btn { padding: 12px 24px; border-radius: 12px; border: none; font-weight: 600; cursor: pointer; }
    .camera-btn.primary { background: var(--primary); color: #000; }
    .camera-btn.secondary { background: rgba(255,255,255,0.1); color: #fff; }
    .toast {
      position: fixed; top: 20px; left: 50%; transform: translateX(-50%);
      background: rgba(0,0,0,0.8); color: #fff; padding: 12px 20px; border-radius: 12px;
      border: 1px solid var(--primary); z-index: 200;
...
const ChatSidebar = ({ chats, activeChatId, onSelect, onCreate, onDelete, onRename, onPin, onFavorite, collapsed, mobileOpen, setMobileOpen }) => {
  ...
  return (
    <div className={`sidebar ${collapsed ? "collapsed" : ""} ${mobileOpen ? "mobileOpen" : ""} ${typeof window !== 'undefined' && window.innerWidth <= 768 ? "mobile" : ""}`}>
      ...
      <div className="sidebar-footer">ALOP-AI • Built different</div>
    </div>
  );
};
...
const AuthenticatedApp = () => {
  ...
  const capturePhoto = () => {
    ...
    c.toBlob(b => {
      setAttachments(prev => [...prev, new File([b], `camera-${Date.now()}.png`, { type: "image/png" })]);
      stopCamera();
    }, "image/png");
  };
  ...
  return (
    <div className="app-root" style={{ "--primary": primary, "--border": border, "--glow": theme.glow }}>
      <GlobalStyles />
                  ...
                  <div className="setting-row">
                    ...
                    <div className="theme-grid">
                      {Object.keys(THEMES).map(k => (
                        <button key={k} onClick={() => setThemeKey(k)} className={`theme-card ${themeKey === k ? "selected" : ""}`} style={{ textTransform: "capitalize" }}>{k}</button>
                      ))}
                    </div>
                  </div>
                  <div className="setting-row">
