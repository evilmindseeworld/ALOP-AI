...
1. **DNS rebinding remains a real validation/fetch TOCTOU.** `assertSafeUrl` returns a vetted `{address,family}` (`backend/lib/url-guard.js:127-178`), but `checkLinks` discards it (`backend/lib/link-check.js:132-146`), and `fetchPageHead` validates then fetches the hostname again (`backend/server.js:804-836`). A DNS owner can answer public during validation and private during connection, creating a blind GET primitive. The link tool exposes only a collapsed verdict and no private response body, and no unauthenticated state-changing internal GET endpoint was identified, so claiming metadata theft or internal compromise would exceed the evidence. Pin every hop to the vetted address with a custom Undici lookup/dispatcher while preserving Host and TLS SNI.
